...
function _mergeFile(base, proposed, override) {
  ...
  setIfValue(
    result,
    'license',
    overrideStrategy(proposed.license, p => SPDX.merge(p, base.license, 'AND'))
  )
  setIfValue(
    result,
    'attributions',
    overrideStrategy(proposed.attributions, p => _mergeArray(base.attributions, p))
  )
  setIfValue(
    result,
    'facets',
    overrideStrategy(proposed.facets, p => _mergeArray(base.facets, p))
  )
  setIfValue(
    result,
    'hashes',
    overrideStrategy(proposed.hashes, p => _mergeObject(base.hashes, p))
  )
  setIfValue(
    result,
    'natures',
    overrideStrategy(proposed.natures, p => _mergeArray(base.natures, p))
  )
  return result
}
